...
Page({
  ...
  onLoad: function (options) {
    ...
    var that = this
    wx.request({
      url: 'https://www.nanhuaren.cn/vcard/catalog/get',
      data: { userId:userId,catalogId: catalogId },
      header: {
      ...
      success: res => {
        ...
        if (res.data.code != 0) {
          var catalog = res.data.data
          var catalogPictures = []
          if (catalog.catalogPicture != null && catalog.catalogPicture != '') {
            catalogPictures = catalog.catalogPicture.split(',')
          }
          that.setData({ catalog: catalog, userId: userId, catalogId: catalogId, catalogPictures: catalogPictures })
  ...
  formSubmit: function (e) {
    ...
    catalog.id = this.data.catalogId
    ...
    wx.request({
      url: 'https://www.nanhuaren.cn/vcard/catalog/modify',
      data: catalog,
      header: {
        'content-type': 'application/json' // 默认值
      },
        ...
        for (var i = 0; i < tempFilePaths.length; i++) {
          wx.uploadFile({
            ...
            success: function (res) {
              ...
              that.setData({ catalogPictures: catalogPictures })
              that.setCatalogPictures()
